...
def save_manufacturer_logo(
    *,
    ...
    suffix_part = f"-{name_suffix}" if name_suffix else ""
    stored_name = f"{safe_name}{suffix_part}-{secrets.token_hex(4)}{suffix}"
...
def validate_stored_logo_path(stored_path: str) -> str:
    ...
    if not normalized:
        raise ValueError("Manufacturer logo path is empty")
    ...
    if normalized.startswith(LOGO_PATH_PREFIX):
        ...
        if normalized != filename:
            raise ValueError(f"Unexpected manufacturer logo path: {stored_path!r}")
