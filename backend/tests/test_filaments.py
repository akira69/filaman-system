...
class TestManufacturerCRUD:
    ...
    async def test_upload_manufacturer_logo_persists_and_serves_file(self, auth_client, db_session, monkeypatch, tmp_path):
        import app.services.manufacturer_logo_service as manufacturer_logo_service

        client, csrf_token = auth_client
        manufacturer = await _create_manufacturer(db_session, name="LogoMaker")
        monkeypatch.setattr(manufacturer_logo_service, "MANUFACTURER_LOGO_DIR", tmp_path)
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["logo_file"].startswith("manufacturer-logos/")
        assert data["logo_url"] == f"/api/v1/manufacturers/{manufacturer.id}/logo"

        stored_file = tmp_path / Path(data["logo_file"]).name
        assert stored_file.exists()
        assert stored_file.read_bytes() == _MINIMAL_PNG_BYTES

        logo_response = await client.get(data["logo_url"])
